...
ReversedRedBlackTree::ReversedRedBlackTree() : root(nullptr) {}
ReversedRedBlackTree::~ReversedRedBlackTree() {
    destroyTree(root);
}

void ReversedRedBlackTree::PrintTree() {
    reversePrintTreeHelper(root, 0);
}
void ReversedRedBlackTree::reversePrintTreeHelper(Node* root, int space) {
    constexpr int COUNT = 5;
    if (root == nullptr)
        return;
    space += COUNT;
    reversePrintTreeHelper(root->left, space);
    std::cout << std::endl;
    for (int i = COUNT; i < space; i++)
        std::cout << " ";
    std::cout << root->data << "(" << root->color << ")";
    reversePrintTreeHelper(root->right, space);
}